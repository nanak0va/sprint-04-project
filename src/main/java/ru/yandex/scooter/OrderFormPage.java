...
import static ru.yandex.common.Utils.waitForElementBeClickable;
import static ru.yandex.common.Utils.waitForTextToBePresentInElement;

public class OrderFormPage {

    private static final By COMMENT_INPUT = By.xpath(".//input[@placeholder='Комментарий для курьера']");
    private static final By BLACK_CHECKBOX = By.id("black");
    private static final By GREY_CHECKBOX = By.id("grey");
    private static final By DATE_INPUT = By.xpath(".//input[@placeholder='* Когда привезти самокат']");
    private static final By DATA_INPUT_CALENDAR = By.xpath(".//div[contains(@class, 'react-datepicker__day--keyboard-selected')]");
    private static final By RENTAL_PERIOD = By.xpath(".//div[@class='Dropdown-control']");
    ...
    public void selectMetroStation(String stationName) {
        orderForm.findElement(METRO_STATION_INPUT).sendKeys(stationName);
        String xpath = String.format(METRO_STATION_SELECT, stationName);
        waitForElementBeClickable(driver, orderForm.findElement(By.xpath(xpath))).click();
    }
    ...
    public void setToday() {
        orderForm.findElement(DATE_INPUT).click();
        chooseNow();
    }
    ...
    public boolean waitForShowOrderFirstStep() {
        return waitForTextToBePresentInElement(driver, ORDER_FORM_HEADER, ORDER_FORM_HEADER_STEP_1_TEXT);
    }

    public boolean waitForShowOrderSecondStep() {
        return waitForTextToBePresentInElement(driver, ORDER_FORM_HEADER, ORDER_FORM_HEADER_STEP_2_TEXT);
    }

    public boolean waitForShowOrderModalFormConfirm() {
        return waitForTextToBePresentInElement(driver, ORDER_MODAL_HEADER, ORDER_MODAL_HEADER_TEXT);
    }

    public boolean waitForOrderModalFormConfirmed() {
        return waitForTextToBePresentInElement(driver, ORDER_MODAL_HEADER, ORDER_MODAL_HEADER_CONFIRMED_TEXT);
    }
}
